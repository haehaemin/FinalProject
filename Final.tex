 	...
 	\begin{equation}
  	...
  	\end{equation}
   	Then, the limit
        ...
        \end{equation}
	||\tilde{L}_2m^{(r)}(\Lambda_R, \Lambda_0)-\tilde{L}_2m^{(r)}(\Lambda_R, \infty)||
 	&<= \Lambda_R^{4-2m}(\frac{\Lambda_R}{\Lambda_0})^2)P^{2r-m}(ln(\Lambda_0/\Lambad_R)),\quad r+1-m\geq 0
  	&= 0, \quad r+1-m<0,
...
\begin{lemma}
	...
	\end{equation}
\end{lemma}
Initial value of $\Lambda^{2m-4}L_{2m}$ is defined by V_\text{int} in eq (4.2), so LHS of above lemma vanishes at $\Lambda_0$ for $m\geq 3$, for $m=2, p\geq 1$, and for $m=0, p\geq 3$. The lemma is trivially true for $r=0$, as 
