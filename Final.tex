...
\end{theorem}
To illustrate, let's consider an example where you don't have a limit. This is the Polchinsky's toy model we saw \ref{sec2}, and similar to what we did earlier, we're going to linearize it, assuming beta is small, and build an expression.
\begin{align}
	\begin{split}
...
$M$ Each component of M represents a derivative of beta. Assuming each component is very small, we can now diagonalize the matrix and solve the differential equation, You should get $\lambda_4(\mu)~\lambda(\mu)\frac{\Lambda^2}{\mu^2}$ as a result.
...
Someone might be very suspicious of the $\ㅣambda\rightarrow\infty$ limit. This is because, as we know, the mass of a scalar particle has a shift corresponding to the ultraviolet cutoff $\Lambda^2$ and has no continuum limit.
%But before we do that, it might be helpful to finish the conclusion of Section 4.2.
%\subsection{Wilson's famous $\epsilon = 4-d$ expansion around the fixed point}
