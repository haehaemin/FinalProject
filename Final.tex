...
We can already feel the traces of the Callan-symmanzik equation here. 
...
\begin{align}
	\begin{split}
		\langle\phi(x_1;\Lambda)...\phi(x_n;\Lambda)\rangle=\frac{\int \mathcal D\phi(x;\Lambda)[\phi(x_1;\Lambda)...\phi(x_n;\Lambda)]e^{-S[\phi(x;\Lambda),\Lambda]}}{\int \mathcal D\phi e^{-S[\phi(x;\Lambda),\Lambda]}}\\
...
\end{equation}
Define anomalous dimension as $\gamma_\phi=-\frac{d\ln Z_\phi^{1/2}}{d\ln \mu}$. 
This is called \textbf{anomalous} because this is responsible to kind a failure of Wald identity of scale transformation in case of correlation function.
Subtract the LHS from the RHS, then one get the
\begin{equation}
	\bigg(\frac{\partial}{\partial \ln\mu} + \beta_i\frac{\partial \lambda_i}{\partial \ln\mu}  + n\lambda_\phi\bigg)\langle\phi(x_1;\mu)...\phi(x_n;\mu)\rangle=0
\end{equation}
